...
class MYDataSet(Dataset):
    def __init__(self, src_data_path, dst_data_path, target_size=(512, 512)):
    ...
    def __len__(self):
        return min(len(self.train_A_imglist), len(self.train_B_imglist))
    
    def __getitem__(self, index):
        train_A_img_path = self.train_A_imglist[index % len(self.train_A_imglist)]
        train_B_img_path = self.train_B_imglist[index % len(self.train_B_imglist)]

        train_A_img = cv2.imread(train_A_img_path)
        train_B_img = cv2.imread(train_B_img_path)

        train_A_img = cv2.resize(train_A_img, self.target_size)
        train_B_img = cv2.resize(train_B_img, self.target_size)
